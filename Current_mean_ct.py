...
class Current_Mean_CT():
    ...
    def machine_design_value(self, df):
        avg_current = df[df['SCS'] == 1]["Mean_current"].mean()
        return avg_current
    ...
    def output(self):
        try:
            ...
            b = pd.DataFrame(df.groupby("DeviceID").apply(self.machine_design_value)).astype(np.float64).reset_index()
            b = b.rename(columns={"DeviceID": "DeviceID", 0: "avg_design_current"})
            ...
            print('Exception in current_mean script:'+str(e))